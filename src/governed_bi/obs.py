...
- **LangSmith** needs no code here. Set ``LANGCHAIN_TRACING_V2=true`` and
  ``LANGSMITH_API_KEY`` and LangChain/LangGraph emit traces automatically, so the
  whole chat run (under ``langgraph dev`` / Platform) and every model call are
  traced with zero wiring. This is the LangChain-native path.
...
def langsmith_enabled() -> bool:
    ...
    tracing = os.environ.get("LANGCHAIN_TRACING_V2", "").strip().lower() in _TRUTHY
