...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        ...
        help=(
            ...
            # a bare one followed by a letter is a format code: ``48% crashed`` parsed
            # as ``%c`` and made ``--help`` itself raise ``TypeError: %c requires int
            # or char`` — the driver's entire CLI surface was unreadable at any width.
            # ``test_help_renders`` renders the whole help text so a reintroduction
            # fails at commit time rather than the next time someone types --help.
