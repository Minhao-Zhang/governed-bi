...
disagreed in the last digit at 008's own inputs. A synonym is how that survived: the singleton
gate is keyed on the name ``mde``, so a second implementation spelled differently sat beside a
declared singleton and no rule looked at it. ``eval/report.py`` reaches down into ``measure/``
for ``mcnemar`` the same way.
...
computed from ``n`` and a base rate alone is instead. Read it before quoting a number from
here; the warning governs every caller and is deliberately not restated.

**``discordant`` is an estimate, not a measurement, when used here.** It is the count of
paired questions where the two arms disagreed, and that count is only known once a comparison
has actually run -- so a pre-check has nothing but a comparable prior run to estimate it from.
The only prior available is experiment 008's ``beer_factory`` pair: 20 discordant of 131
questions (15.3%), which is the 0.0956 this gate is calibrated against. Presenting the MDE this
produces as an exact figure, rather than as an estimate carried forward from that one prior
run, is the mistake this docstring exists to head off.
"""
