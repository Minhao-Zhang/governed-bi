...
RECORD_REGISTER: tuple[RecordField, ...] = (
    ...
    _f("failure_cause", Tier.outcome, Absence.not_applicable, Stage.stamp,
       ...
       "`computed_correct` is separate from `correct` (`eval/harness.py`: 'one merge of the two "
       "and the artifact silently reports an engine that commits to everything'). `error_type` "
       "is declared as the exception CLASS of a turn that RAISED, and a consumer reading "
       "`error_type is not None` as 'this turn crashed' is reading the declaration; writing "
       "taxonomy strings into it took that count from 0 to 78 on experiment 008's baseline and "
       "made the field mean two incompatible things. Null when the turn is not "
       ...
       "is written by `eval/harness.py::project_turn`, which is the only place both the "
