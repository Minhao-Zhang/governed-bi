...
F1 tests that inject ``facet_route_hits`` keep working.
"""
...
def _channels_for(stage: Stage) -> dict[str, str]:
    """Channel map from :func:`expected_channel_state` (configured → ``ran``)."""
    out: dict[str, str] = {}
    for ch in Channel:
...
def _run_facet(
    ...
    if index is not None:
        ...
        # Stub extraction: same question as query, empty hits (no model).
        hits = []
    else:
        hits = _hits_from_hook(state, stage, question)

    return {"facets": {stage.value: _facet_result(stage, question, hits=hits)}}
