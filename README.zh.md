...
- **两套 harness，一个基座（substrate）。** `curator`（构建）从一批已知良好的 `(question, SQL)` 种子对中*生成*语义层（corpus）；`server`（服务）*使用*它来回答问题。二者风险特征相反，却共享同一个 corpus。
...
[演练](docs/walkthrough.zh.md)是一份引导式的「第一个问题」教程，[快速上手](docs/usage.zh.md)是完整参考。要直接用 Python 驱动 agent 核心，见 [`docs/server.md`](docs/server.zh.md)。
...
`useStream`）。它面向本后端的流式对话契约开发，但还没有端到端实测接通。
...
[server](docs/server.zh.md) · [viz](docs/viz.zh.md) · [术语表](docs/glossary.zh.md)。
