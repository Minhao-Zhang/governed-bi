...
*已实现：* 两套 harness 都由一次普通的 `uv sync` 安装（不需要任何 extra），构建在基于 LangChain 的模型客户端之上。Server 就是 [ADR 0002](adr/0002-governed-agentic-serve-runtime.md) 所定义的受治理 agentic 内核 `server.agent`（薄薄的外层确定性轨道，包裹起一个由中间件治理的 `create_agent` 推理循环）——自 P2 切换以来它是唯一的服务路径；早先的确定性流程（`server.flow::answer_question`）与那个陈旧、未被使用的 `server.graph` DAG 均已删除。没有实时模型时会失败即拒：`build_stack()` 仍可离线构建以支撑只读审计 API，但服务进程会在启动时报错，`/chat` 返回 503，直到配置好模型为止。相对于（现已移除的）确定性流程的首次线上 A/B 促成了 [ADR 0002](adr/0002-governed-agentic-serve-runtime.md) 的 Amendment 1；当前评测数字见 [三臂结果](plans/three-arm-experiment-results.md)。curator = `curator.deep_agent`（一个运行在 Facts 画像（profiling）与只读探测（probe）工具之上的 deepagents agent，其构造已离线验证，实际运行则受模型门控）。
...
按 D15，在多 schema 的 Postgres / Redshift 路径上，一个连接感知（join-aware）的 schema 路由器会先于 RVGD 检索运行，因此检索会跨越多个 schema；单 schema 路径则跳过它。**已落地**（`retrieval.schema_router`；已接入 `server.agent`）。
...
> 约束；预算耗尽即失败即拒。经过修复后的答案会被标记为 `lineage`，而不是
> `governed`。
>
...
- 按 EX 打分的臂阶梯（见 [D14 修订](design-decisions.md#d14-sme-growth-benchmark-on-bird-obfuscation)，2026-07-15）：（1）无层下限；（2）自主 curator；（3）按轮次的 SME——外加一个作为虚线*可恢复上限*的**测试感知 SME 预言机**。去混淆"gold"预言机已**退役**（它从来不是真正的上限——curator 技能就能超过它）。**护城河（moat）= curator 挽回的那部分因混淆导致的准确率下降。**
