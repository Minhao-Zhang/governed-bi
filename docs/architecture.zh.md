...
2. **服务（serve）的*权限（authority）*是确定性的；其*推理（reasoning）*则可以是 agentic 的。** 问题可以很宽泛，但 SQL 必须收窄。权限（什么可以执行、什么被信任、什么会被记录）是硬编码且可审计的；而寻找答案的推理，则可以作为一个有界的 agentic 循环运行，并被限定在受治理的只读工具之内。[ADR 0002](adr/0002-governed-agentic-serve-runtime.md)（Proposed）推翻了本脊柱早先「绝不是自主式 ReAct 循环」的表述，把推理层面的自主性与权限层面的自主性区分开来；只有后者仍被禁止。
...
*已实现：* 两套 harness 都存在于 `agents` extra 之后，构建在基于 LangChain 的模型客户端之上。Server 当前是确定性流程（`server.flow::answer_question`，即代码默认）；`server.graph` 则是一个陈旧、未被使用的 LangGraph `StateGraph` DAG（曾与 `answer_question` 在答案上等价，此后已发生漂移），计划删除。[ADR 0002](adr/0002-governed-agentic-serve-runtime.md) 方向（Proposed）是第三条路径 `server.agent`（薄薄的外层确定性轨道，包裹起一个由中间件治理的 `create_agent` 推理循环），已落地于 `agent_serve` 配置开关之后（默认关闭）；两条服务路径都调用同一个共享的治理内核。相对于确定性流程的首次线上 A/B 记录于 [agentic-serve A/B 结果](plans/agentic-serve-ab-results.md)。curator = `curator.deep_agent`（一个运行在 Facts 画像（profiling）与只读探测（probe）工具之上的 deepagents agent，其构造已离线验证，实际运行则受模型门控）。
...
> 是 `governed`。
>
> 在 [ADR 0002](adr/0002-governed-agentic-serve-runtime.md) 的 agentic 路径上
> （Proposed；`agent_serve` 开关），这个手写的 `while attempts < 3` 循环被 agent
> 自身的工具反思循环（tool-reflection loop）取代：`run_query` 是一个受治理的只读
> 工具，其每轮的尝试次数上限在 `wrap_tool_call` 中强制执行，失败会作为 `ToolMessage`
> 返回给 agent 供其反思。护栏与标记保持不变（同一个共享治理内核在工具边界上运行），
> 因此自主性只在*如何找到答案*上放宽，绝不在什么可以执行上放宽。

> **治理账本（governance ledger，agentic 路径）**
>
> 在 [ADR 0002](adr/0002-governed-agentic-serve-runtime.md) 路径上，执行与审计共享
