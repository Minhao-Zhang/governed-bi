...
| serve（智能体核心：路由、上下文、受治理的工具、护栏、自修复、缓存、标记） | 可运行，需要实时模型 | `src/governed_bi/server/` |
...
| 智能体 harness（LangGraph 受治理 serve 核心、deepagents 的 curator） | 可运行（由一次普通的 `uv sync` 安装，不需要任何 extra） | `server/agent.py`, `curator/deep_agent.py` |
| Postgres / Redshift 连接器 | 已实现（基于 psycopg，由一次普通的 `uv sync` 安装）；离线测试，未连真实库 | `src/governed_bi/gateway/connectors/` |
...
Postgres（`information_schema`）与 Redshift（`svv_*`）也已实现（两者都基于 psycopg，
由一次普通的 `uv sync` 安装），并有离线单元测试，但尚未连过真实服务器。将它指向一个 SQLite 文件，
