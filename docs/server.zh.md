...
4. **SQL 语义缓存快速路径**：问题嵌入 → 与缓存 SQL 的余弦相似度 ≥0.92 → 命中则跳过检索（retrieval）/规划/生成，但**始终重新执行**（仅 SQL 文本、以用户身份、D7）。TTL 15 分钟；成功后写回。*已实现：* `server.cache.SqlCache`（默认关闭，以注入方式提供）。命中的缓存条目还会针对其存入时所持有的已授权表**重新过护栏**，然后再执行；已过期或现已被拦截的命中会回落到完整流水线（失败即拒）。只有 `governed` 档位的答案才会被写回。
...
7. **SQL 生成**（一个可插拔的扩展点（pluggable seam），`SqlGenerator`）：设计愿景中的生成器在 LLM 之上叠加一个系统提示（角色 → schema 约束 → 安全 → 输出），输出**经过混淆的物理标识符**。*已实现：* `TemplateSqlGenerator`（确定性，单表 metric 聚合，不依赖模型）以及 `LlmSqlGenerator`（读取 `PromptContext`，调用注入的 `ChatClient` = OpenAI `gpt-5.5` low，对修复循环具有反馈感知能力，并以 `CANNOT_ANSWER` 哨兵值拒答）。运行哪一个由注入方式决定。
...
10. **应答与可靠性标记（reliability stamp）**：尽力而为的分档（governed → lineage → fenced-raw）。高风险（high-stakes）→ 签核（sign-off）/ 仅 SQL（SQL-only）。

**自修复（第 7-9 步构成一个有界循环）。** 生成、护栏与执行以循环方式运行：护栏拒绝或执行报错会反馈给生成器，供其再次尝试，每次尝试都会重新过护栏，因此未经审查的 SQL 永远不会被执行。当生成器已无法再改进（重复输出同一条查询）时会提前停止，并在达到一个较小的次数上限后失败即拒。经过修复的答案会被标记为 `lineage`，而不是 `governed`。这能够从畸形或超出范围的 SQL 中恢复，且从不放行未经检查的查询；但它无法捕捉*看似合理却错误*的 SQL（语法有效、在许可清单内，但计算逻辑是错的），这正是可靠性标记以及拒答 / 仅 SQL 路径存在的原因。护栏是安全/治理层面的关卡，不是正确性的判定者（oracle）。
...
**不确定性汇聚 → 可靠性标记：** 使用了低置信度连接 · fenced-raw 回退 · 触发了 Corrective-RAG · suspect 列在范围内 · SQL 经过修复 → 档位下调 → 差异化处理（D5，让标记真正具有约束力）。这些档位是**未经校准的治理/不确定性启发式**，需要在评测中调优：`governed` 意味着安全、在范围内，且没有触发任何不确定性标记，**不**意味着已验证正确。由于失败即拒本身带有误拒代价，评测中的 `false_refusal_rate`（见 [Architecture](architecture.zh.md) 第 8 节）正是用来制衡它的指标。
