...
| 2 | EX comparator | `2d4b52a` | BIRD-Obfuscation's `normalise_result` transcribed. `result_fingerprint` is now **byte-identical** to `hash_normalised_result`, verified on six rowsets. `EX 0.049` retired via `RETIRED_CLAIMS`; grep gate verified to fire on reinsertion. <!-- [retired] --> |
