...
3. [Asset schemas](v1/asset-schemas.md): the per-asset YAML field spec (Facts / Inference / Audit tiers).
4. [Curator](v1/curator.md): the build-side proposer + adversary loop. For the exact prompts, see [Curator LLM-call walkthrough](v1/curator-llm-call.md).
5. [Analyst](v1/analyst.md): the serve-side governed agentic core + guardrails. For the exact prompts, see [Analyst LLM-call walkthrough](v1/analyst-llm-call.md).
6. [Viz](v1/viz.md): the audit surface — the presenter view models plus the `governed_bi.api` HTTP API to browse the layer and chat with the governed Analyst (corpus write gated by `allow_edit`; the interactive UI is a separate project).
7. [Measurement](v1/measurement.md): what the eval harness records and where a failure localises — read this when a number looks wrong. The field-by-field register is [Eval metrics](v1/eval-metrics.md).
8. [Prompt-variant experiments](v1/prompt-experiments.md): the prompt registry, how a run selects a variant, what gets stamped where, and how to decide which variant a measured failure actually calls for.
...
- [Corpus authoring](v1/corpus-authoring.md): write and validate corpus assets step by step.

Two step-by-step call traces sit alongside the design docs, useful when reading
the code: [analyst sequence](v1/analyst-sequence.md) and
[curator sequence](v1/curator-sequence.md).
...
| [0011 Two models, and a query per facet](adr/0011-two-model-split-and-facet-query-rewriting.md) | Accepted 2026-08-04; built — `Session.utility_model` + `llm_utility_model`, five registered facet rewriter prompts. Amends 0005 §2.3, whose facet table has `schema` and `example` calling no model |
...
> down in
> [`plans/experiment-runbook.md`](v1/plans/experiment-runbook.md#the-result-that-would-make-us-abandon-the-corpus-thesis).
...
- [Serve transparency handoff](v1/plans/serve-transparency-handoff.md): making the agent's *inputs* visible in the UI. The earlier design note [`serve-transparency.md`](v1/plans/serve-transparency.md) is marked **SAFE TO DELETE** (no unique content; live event contract is [analyst.md](v1/analyst.md)). Contract publication continues under [rebuild-checklist.md](v1/plans/rebuild-checklist.md) §5.3.
- [SME channel repair](v1/plans/sme-channel-repair.md): why the `curated_sme` arm moved nothing, and the fixes in order.
- [Eval rebuild](v1/plans/eval-rebuild.md): why every pre-2026-07-26 number is discarded, and the four fixes that follow.
- **[Near-term plan](v1/plans/near-term-plan.md) (Simplified Chinese): the seventeen items being implemented now, cut out of the checklist below on two tests — delegable, and machine-checkable. Five milestones ending in one quotable run. Says what a reviewer will reject on, and lists what is deliberately *not* in this batch. The entry point for anyone picking up work today.**
- **[Rebuild checklist](v1/plans/rebuild-checklist.md) (Simplified Chinese): the current work queue. Eleven cross-cutting items in dependency order, plus the analysis-tooling, recording, observability, signal, and run tracks. Each item says what to change, which files it touches, and how to verify it. Start here.**
- **[Rebuild decisions](v1/plans/rebuild-decisions.md) (Simplified Chinese): the twenty-two decisions behind that queue — what was chosen, why, and what was rejected. Read this before overturning anything in the checklist.**
- [Build sequence](v1/plans/build-sequence.md): **superseded** by the two above, kept as the evidence index — it maps each item back to the analysis that found it (62 items → 41, four phases, plus the non-goals).
- [Grill agenda](v1/plans/grill-agenda.md) (Simplified Chinese): the pre-grill briefing — the five analyses turned into eight contested decisions with the sharpest objection to each option. **Consumed by the 2026-07-30 grill**; its conclusions live in `rebuild-decisions.md`.
- [Multi-turn adversarial](v1/plans/multi-turn-adversarial.md): every number in this repo is turn 1. Routing and retrieval run on the coreference-unresolved question, and AUDIT S4's bound makes a mis-routed follow-up unrecoverable — a correct follow-up can be refused.
- [Governance red team](v1/plans/governance-red-team.md): attacking ADR 0002's topology-not-trust claim. Seven hypotheses, each with the test that settles it; the graded-delivery path re-checks with the scope argument disabled.
- [Corpus drift](v1/plans/corpus-drift.md): drift is checked once at build time and never again; `/health` runs the validator with no connector, so it cannot report the failure it implies.
- [Framework + logging audit](v1/plans/framework-and-logging-audit.md): best-practice scan of every LangGraph / LangChain-middleware / DeepAgents / Langfuse / checkpointer / memory site against the skills and vendor docs **as of 2026-07-30**, plus the one-identity design for logging everything. Its headline was a privacy finding — the Langfuse content mask did not cover callback data — which is moot: Langfuse was removed on 2026-08-02, and traces now log in full by decision (`governed_bi/obs.py`). Read the Langfuse sections as history.
- [Book fidelity assessment](v1/plans/book-fidelity-assessment.md): this repo measured against the reference document it grew out of — which divergences were deliberate (9), which are drift nobody decided (15), and which gaps the reference shares. Focused on routing, the semantic layer, and a nine-axis systematic analysis of RVGD retrieval.

Two plans finished and were deleted; their wire contracts now live in
[Analyst](v1/analyst.md) — the [governance event stream](v1/analyst.md#the-event-contract-per-step)
and [serve-time clarification](v1/analyst.md#serve-time-clarification-hitl). The module
deepening plan was deleted unstarted; the items from it that still matter are in
[open work](v1/open-work.md), and the rest is in git history.

Closed trackers and superseded plans are not kept as files — git history holds
them. Open items from all of them live in [open work](v1/open-work.md).
...
| Doc | Package area |
|---|---|
| [Asset schemas](v1/asset-schemas.md), [Design decisions](design-decisions.md) D9 | `src/governed_bi/corpus/` |
| [Curator](v1/curator.md) | `src/governed_bi/curator/` |
| [Analyst](v1/analyst.md), [Architecture](architecture.md) §6 | `src/governed_bi/analyst/`, `gateway/`, `graph/`, `retrieval/`, `memory/` |
| [Architecture](architecture.md) §8, [Measurement](v1/measurement.md) | `src/governed_bi/eval/`, `src/governed_bi/stages.py` |
| [Prompt-variant experiments](v1/prompt-experiments.md) | `src/governed_bi/prompts/` |
