...
7. `summary.json` → `arms.<arm>.by_tier`、`by_semantic_assurance`、
   ...
   重心从 `unflagged` 推向 `unverified` 或 `none`，或者靠交付更多低于可靠性门槛
...
## 什么结果会让我们放弃 corpus 这个论断

写在跑批之前，因为一个只能给出一种答案的 harness 算不上一次实验（AUDIT C8）。
**一句话说清楚：**

> 如果在**测试**切分上、限定在无孪生题的那一层、以 69 个 schema 为规模，
> `seeded → curated` 这一步的配对 McNemar 点估计低于 **+2.0 个 EX 百分点**，
> Holm 校正后的 *p* > 0.05，而且这一结论在**三次独立的 curator 抽样**（同一批
> 输入、不同 agent seed，各自重新构建一遍）上都成立，那么 LLM curator 就没有
> 挣回它的成本，corpus 是护城河这个论断在这个规模上就不成立。

每一个词都是可机械核对的，而且现在就能算出来：

| 术语 | 出处 |
|---|---|
| arm 对 | `seeded → curated`——把 LLM curator 从那趟免费的确定性处理里单独隔离出来的那一档。不是 `baseline → curated`，那会把两个机制捆在一起。 |
| 指标 | `comparisons[].net_questions` / `ex_lenient` 的配对差值（`eval.analysis`），而不是跨运行相减的点估计 |
| 分层 | 无孪生题的那一层（见上文「引用不含孪生题的那一层」）——训练集里存在 gold SQL 孪生时，召回和泛化是分不清的 |
| 效应量 | +2.0 个 EX 百分点。低于约 +1.6 个百分点这条 MDE 机制认为可分辨的区间，一次「赢」就和解码噪声分不开。 |
| curator 抽样次数 | 三次。在一个随机化的 agent 上取 `n=1`，只是从处理分布里抽了一个样本——这一点 `power.py` 自己的 docstring 里就写着。 |

这条判据有两点要老实说清楚：

- 它是一句关于**这个基准测试、这个规模、这个模型**的话，不是关于语义层这
  类东西本身的一般性结论。这里得出的零结果，不能说明策展帮不上忙；只能说明
  *这个* curator，在混淆后的 BIRD 上，没能挣回它的成本——而这正是这个项目
  实际要做的决定。
- 它所依赖的 MDE 来自**serve**端的噪声（把同一份语料库重新服务一遍），而
  处理本身是**corpus 级别**的。所以这个界是偏乐观的：它量的是解码方差，不
  是 curator 方差。三次抽样的要求就是为了补上这个缺口，而在真的跑出三次抽样
  之前，这条判据无法被评判。

可引用性闸门执行的是这件事对称的另一半：一次跑批里，正确的那些行如果大多是
免费白拿的（gold 为空、没有 `FROM`、和 gold 表零重叠），现在会被判成不可
引用——一个好看的结果，和一个崩掉的结果一样，都可以因此被判不合格。

## 这条阶梯能告诉你什么，不能告诉你什么

每一步都是**相邻**的，但相邻不等于只改了一件事——见每条比较记录上的
`mechanisms_changed`，以及 `single_variable`：它现在的意思是恰好改了一个
机制，而不再只是恰好隔了一档：
