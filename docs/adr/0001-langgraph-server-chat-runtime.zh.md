...
**LangGraph `StateGraph`**（`analyst.graph.build_serve_graph`），在答案上与朴素的
`answer_question` 等价。
...
- ~~**`ServeState` 必须支持检查点序列化。**~~ **(实现阶段以另一种方式化解了。)**
  ...
  **chat** 图,持久化的 state 只有 `{messages, answer}`(两者都能 JSON 序列化),整条
  ...
  的 `retrieval`/`context`)只当节点内的局部变量,绝不进检查点,所以 `analyst/flow.py`
  和 `analyst/graph.py` 都没动,`answer_question` 与图的等价关系依然成立。
  *（前向指引：这种确定性单节点的描述是本决策当时的运行时状态；serve 之后已
  切换到 [ADR 0002](0002-governed-agentic-serve-runtime.md) 所述的受治理
  agentic 核心，取代了 `answer_question`/`analyst/flow.py` 以及那张陈旧的
  `analyst/graph.py` DAG。）*
