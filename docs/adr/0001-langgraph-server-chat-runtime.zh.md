...
**负面 / 代价**
- **`ServeState` 必须支持检查点序列化。** 它目前存放着一些活对象
  （`networkx` 图、gateway 许可清单、pydantic 的 `retrieval`/`context`/
  `generated`）。被持久化的 channel 必须是可序列化的。重量级对象要移到部署
  依赖 / 按节点重建当中；只有消息与轻量级结果会被写入检查点。图与
  `answer_question` 之间的等价关系（在测试中被断言）必须保留。这是一项真实、
  非同小可的工作。
