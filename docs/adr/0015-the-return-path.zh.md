...
> **状态，2026-08-23：第 0-6 步已接受并建成于 `design/return-path`。** 建设过程中的**五项**实测改变了本文
> 的决定；证据在 `docs/open-work.md` §3.10a-3.10c，此处刻意不重复 —— 本文件是决定记录，那一份是工作
> 清单。
>
> | 实测 | 它改变了哪个决定 |
> |---|---|
...
- **状态：** 提议中 (Proposed)，2026-08-23。**无代码。** 本文中所有模块路径、签名和路由都是设计，
  不是描述；除文中明说「已存在」的，此处命名的东西都不存在。设计过程：五份独立提案（接入、流水线、
  验证、工作流，以及一份带实测的原型），随后三轮对抗式批判。
- **English:** [0015: The return path](0015-the-return-path.md).
- **决策者：** 项目所有者 + 设计会议（2026-08-23）。
- **配套工作参考：** [回流路径](../return-path.zh.md) —— 构建顺序、数据形状、路由表和测试名。
  本页是决策与推理；那一页是工程师照着实现的东西。
- **阅读须知。** Context 一节里有四条数字是为这次决策专门测出来的，别处任何文档都没有。它们标注为
  **实测**，并给出产生它的命令。除此之外本页不含任何数字。
...
现在有两个界面。`POST /turns/{turn_id}/raised`（`api/clarification_routes.py:66`）让读者对一个已结束的
turn 提交一条笔记 —— `kind ∈ {from_refusal, wrong_answer}`，一段上限
`RAISED_NOTE_MAX_CHARS = 4000` 的自由文本 `note` —— 并把一行追加到 checkpoint 化的累积通道
`ServeState.raised` 上。`GET /clarifications/pending` 把这些 open 行与活跃的 `ask_user` interrupt 并集，
按最早优先展示。

**没有任何东西关闭一个 open 行，也没有任何东西据此行动。** `serve/raised.py::raised_row` 写下
`open: True`，注释是「until a later closer exists」；那个后来的关闭者不存在。UI 告诉读者
"Filed. It is on the pending list."——这句话是真的，而且这就是全部发生的事。
...
**实测：`../MS Fabric Facilities` —— 今天在服务的语料 —— 不是一个 git 仓库。** 它有 `.gitignore`，
...
这正是 `eval/power.py::require_power` 存在的目的：提前拒绝这种事。而它现在没有调用者。
...
`runs/feedback.sqlite`，标准库 `sqlite3`，**同步**，放在新的 `feedback/` 包里。每一条新的 observation
都写在那里。`serve/raised.py`、`api/raised_write.py` 以及
`ThreadTurnLog.append_raised` / `raised_of` 会被删除 —— **但不在同一个 commit 里**，而且分阶段这件事
本身是一个决定，不是谨慎。

一次性删除有一个没设计过的迁移问题：已经躺在 checkpoint 里的行变得不可达，而「没有迁移，那些行不可达」
是一句得有人签字的话。所以：存储先落地并接下所有新写入；通道继续为已有内容工作；
`tools/drain_raised.py` 遍历 thread 把那些行抄进来；在 drain 还有活干的期间，读取方并集两个来源。
**当 drain 报告为零并跨一个 release 保持为零时，删掉这个并集** —— 一个具名的终止条件，因为一个没有
出口的兼容并集，正是两个事实来源变成永久的方式。
...
有一个后果是好事。`api/raised_write.py` 拒绝在暂停的 thread 上提交，因为 `as_node="raise_note"` 会消耗
掉活跃的 `ask_user` interrupt。一旦不再写 graph state，就没有 interrupt 可被消耗，于是
**turn 正处于暂停中的那位读者 —— 最可能想投诉的那一位 —— 可以提交了。**
...
| `superseded` | 哈希已离开 base，而内容不在那里 | 一次 `git apply` 冲突、一次语料 CI 重排、或评审者在提交前改了补丁 —— 三者都正常，而两状态模型会把它们统统静默标成「已交接，永远」，那正是今天关不掉的 `open: true` 在上一层的复现 |

**`closed` 不是一个状态。** 没有任何东西分支于它。`open` 计算为 `state not in TERMINAL`，从不存储，
于是今天那个关不掉的行不再是可表达的东西。
...
| T2 | 树对活体 catalog 做绑定 | 数秒，需要数据库 | 引用了不存在列的 metric（M2） |
...
### 7. 一个 corpus release 是一个被声明的处理变量，而它的 knob 不存在

**实测：`comparability_keys()` 是 50 个名字，没有一个含 "corpus"。** 所以一个处理变量**就是**语料的
...
用新 thread 而不是原来那个，理由是 `api/raised_write.py` 已经详细记录过的「不要写进别人的 thread」，
...
1. **两个新包，以及一个 `LAYERS` 决定。** `tools/check_imports.py::LAYERS` 必须穷举 `src/governed_bi`
   ...
   校验器判断一个补丁，且不得 import `serve`、`govern`、`api` 或 `eval`。`triage` 在 `eval` 之后、
   `api` 之前。
2. **`api/visibility.py::visible()` 覆盖不到新界面，而且覆盖不了。** 它收窄的是一个**语料投影**；
   ...
   `/audit/corpus` 的 `problems` 采取的是同一笔交易。
3. **面向工程师的动词是新权限，它们以未挂载状态发布。** 藏在 `GOVERNED_BI_FEEDBACK_ADMIN` 之后，返回
   404 而不是 403 —— 403 会确认这个路由存在。一个分叉能开启的最便宜控制是**只**给这几个动词加一个
   token，而与 2026-08-13 那次回退不同，它不花任何代价：LangGraph Studio 从不调用它们。
4. **在唯一一个默认启用的动词上，本设计是按算术在收窄。** 一行一次、带配额、可清扫 —— 对比今天那种
   被重新序列化进该 thread 之后每一个 checkpoint、且存储没人清扫的行。
...
7. **一位读者的自由文本在任何人看到它之前经过两次模型调用**，而设计会议关于「暂存树在 `corpus_root`
   之外」即结构性控制的说法**并不完整**。这段文本被 Diagnoser 读到，并经由 diagnosis 转述给 Author；
...
8. **`raised` 通道的行是被迁移，不是被遗弃。** Decision 2 用 `tools/drain_raised.py` 和一个带具名终止
   条件的读取并集买到这一点，代价是多一个构建步骤。替代方案 —— 删掉并宣布那些行不可达 —— 是一句得有人
   签字的话。
...
1. **serve 的处理身份不移动。** prompt 注册表拆分之后，默认变体下的 `prompt_set_hash()` 与
   ...
   正文；`prompt_set_hash()` 不变，`triage_prompt_set_hash()` 移动。
...
8. **修订循环有界。** 一个其暂存 asset 永远通不过 `validate` 的脚本化模型，会在 `max_revisions` 处终止
   并撤回补丁，且恰好调用了那么多次 Author。
9. **drain 有终止条件，并集有出口。** `tools/drain_raised.py` 在一棵有 N 个通道行的树上先报告 N、再报告
   0；读取并集在一个同时把 drain 最后一次非零报告移出记录的 commit 里被移除。
...
- **认证。** 审计发现 A1 和 A7 是开放的，本 ADR 不关闭它们。它增加一道收窄接缝和一个默认未挂载的管理
  界面，并且明说：碰到端口仍然就够了。
- **谁拥有语料仓库的 CI。** 本设计规定了那个 CI 必须跑什么。它没说谁来写，而今天服务的那个语料根本不是
  一个 git 仓库（Context 5）。
