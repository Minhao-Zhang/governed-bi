...
([decisions](plans/rebuild-decisions.md)). Items are not copied here — read this file for
...
| C21 | **Every git reader returns "no provenance" from inside a git worktree, and `dirty` reports that as clean.** `corpus_release_hash`, `git_head_branch`, `git_main_hash` and `working_tree_state` all build `repo_root / ".git" / ...`, but in a linked worktree `.git` is a **file** containing `gitdir: <path>`, not a directory. Reproduced from a worktree at `d29fb16`: `corpus_release_hash() -> "unknown"`, `git_main_hash() -> "unknown"`, `working_tree_state() -> (False, None)`. The last one is the hazard — `dirty=False` means "checked, clean" and "could not read git" indistinguishably, so a run launched from a worktree records a manifest that *looks* traceable and carries nothing. `eval/index.py:388` uses `git_sha` as a comparability / resume-drift key, so two worktree runs of **different** code compare as identical on it. Not triggered today (experiments run in the primary checkout). Fix: when `.git` is a file, follow its `gitdir:` pointer — no `subprocess` needed, the no-subprocess constraint at `provenance.py:173-175` still holds. `working_tree_state` should return unknown, not clean, when it cannot read. Found by the M4 fix agents, which ran in worktrees. | `provenance.py:170-260` |
| C22 | **`api/routes.py` mutates the shared frozen `ServeStack` at import time.** `routes.py:27-29` does `object.__setattr__(get_default_stack(), "can_stream", True)` on the process-wide singleton N14 introduced, so the plain REST app's advertised capabilities depend on whether the LangGraph route module happened to be imported into the same process. This surfaced as test pollution (fixed by an autouse reset in `tests/conftest.py`, `c6c74b1`) but the production coupling is untouched: `can_stream` should be a property of the app being built, not a write onto shared state. | `api/routes.py:27-29`, `api/stack.py` |
...
| # | What | Where |
| --- | --- | --- |
| E12 | **N11's "every line carries a timestamp" is not met for the build-progress lines.** Only `logger.*` calls routed through `configure_logging` get one; the per-db `built corpora:` progress line and the adjacent `*** build FAILED for [db]` still go out via bare `print`. Moving them to the logger also moves them from stdout to stderr, which breaks the sink `test_progress_is_reported_as_each_build_finishes_not_all_at_the_end` counts — so this needs a deliberate decision about which stream build progress belongs on, not a drive-by change. | `eval/run_datalake.py` build loop |
| E13 | **A dead `monkeypatch` line that `raising=False` guarantees will never announce itself.** `tests/test_split_gap.py:176` patches `rd.write_manifest_hook`, and `write_manifest_hook` has **zero** occurrences in `run_datalake.py`. It sets an attribute nobody reads. Delete it next time that file is touched. (Swept the other 14 `raising=False` uses: all `delenv`, all legitimate. Module-attribute `setattr` guards fail loudly with `AttributeError` when an import style changes — which is how the M4 curator-tracing guard announced itself — so this is the only silent one.) | `tests/test_split_gap.py:176` |
