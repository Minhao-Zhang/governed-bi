...
9. **Execute as-user**: row-cap + timeout, forced-LIMIT injection (Postgres / Redshift / SQLite), audit/replay. **Row-level scoping is a deferred seam, not a built control:** `Gateway.execute` takes `identity` and uses it only to write the audit row — it never sets a session role and never scopes rows. An enterprise fork wires RLS there; this repo does not.
10. **Answer + reliability stamp**: a **two-axis** stamp — `safety_clearance` (guardrails + authorization passed, a gate) and `semantic_assurance` (`unflagged` → `heuristic` → `unverified`, whether any uncertainty flag fired — not verified-correct). The single-axis tier (governed → lineage → fenced-raw) is their compact **display-only** projection, never a parallel concept. High-stakes → sign-off / SQL-only.
...
- `sample_rows(table_id, n)`: row preview, executed under the caller's `identity` for the audit record. Only allowlisted columns are returned; row-level scoping is not implemented (see step 9).
