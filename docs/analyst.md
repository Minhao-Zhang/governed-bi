...
**Uncertainty aggregation → `semantic_assurance`:** low-confidence join used · fenced-raw fallback · suspect column in scope · SQL repaired → drops `unflagged` to `heuristic` (or `unverified`) → differential handling (D5, give the stamp teeth). The `corrective_rag` signal is reserved on the stamp dataclass but unused (no serve path writes it). This is the *semantic* axis only; `safety_clearance` is a separate pass/fail gate that says nothing about how right the number is. The levels are **uncalibrated governance/uncertainty heuristics**, to be tuned on the eval: `unflagged` means no uncertainty flag fired, **not** verified-correct and **not** "well grounded in retrieval". Because fail-closed carries a false-refusal cost, the eval's `false_refusal_rate` ([Architecture](architecture.md) §8) is its counterweight.
