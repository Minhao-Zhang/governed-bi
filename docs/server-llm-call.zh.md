# Agentic BI 服务端：LLM 调用全流程

本文逐次调用地追踪一个问题如何流经服务路径（`server.agent`），展示模型在每一步
实际收到的*逐字*文本。它是 [Server](server.zh.md) 的补充。那份文档描述的是周围
的轨道（rails）；这里的目标更窄：把每一条系统提示逐字重现，把每一条 user/human
消息连同动态内容注入处的占位符一起展示，并把工具循环呈现为一份示意性的对话记录。

> 实现：[`src/governed_bi/server/agent.py`](../src/governed_bi/server/agent.py)、
...
`SYSTEM_PROMPT`（逐字，`server/agent.py`）：
...
**另见：** [Server](server.zh.md) 了解完整的轨道/护栏设计；
