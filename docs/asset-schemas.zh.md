...
> 这是单一的最大杠杆。Skill **没有 oracle 对应物**（评测阶梯中任何一个已构建的
> 分支都无法推导出它），因此即便是 `ceiling` 这一档也没有 skill。这正是为什么
> `curated` 分支能在对 skill 敏感的问题上*超越*可恢复上限（recoverable ceiling，
> D4）的原因。
...
当 curator 无法从 Facts + 批次中解决某个问题时，它**记录该问题而非臆测**，作为资产 **Audit 层**上的一个 `clarification` 块。因其属于 Audit 层，它**绝不注入 Analyst 的上下文**（见上文消费契约）——一个未决问题无法泄漏进 SQL 生成或检索。问题未决期间，资产仍从其 Inference 层给出尽力而为的答案（低 `confidence` + 一条 `suspect` 警示）；一个 **Responder**（生产中是人类 **SME**，评测中是 **Simulated SME**）作答，`accept_answer` 将其翻转为 `answered` 并重新盖上 provenance。参见 [D12](design-decisions.zh.md#d12澄清协议)。
...
按领域组织的行文式程序性知识。Frontmatter 携带与 YAML 资产相同的溯源信息（可审计，但没有 gold）。正文会被检索并注入到 Analyst 的 prompt 中。
...
Skill 通过 ID 引用类型化资产，**不会**重述这些资产的数据。skill 完全是 curator 带来的增量价值：没有 oracle 推导出的 skill 可以比对（diff）。
...
- 如今已不存在基于 manifest 的 oracle 填充器来填充 Inference 层。曾经有一个已退役的去混淆 "gold" 分支，从 manifest 中以确定性的方式填充*相同的* Inference 字段：真实名称（通过 rename map）、FK 图（来自原始 schema），以及 manifest 中记录的任何 `reliability.status=suspect` 标记，并附带 `provenance.source: gold`、`confidence: 1.0`——它已被**移除**：它从来都不是真正的上限（ceiling），因为 curator 的 skill 可以在 skill 敏感的问题上超越它。（`ProvenanceSource.gold` 只作为遗留的枚举值保留在 schema 中。）
- Facts 在评测阶梯的每一档中都是相同的（均从 catalog 中读取）；只有 Inference 与 Skill 会变化。
- **Skill（Markdown）只由 curator 产出**：评测阶梯里没有任何其他环节能推导出它们，包括那个已设计但尚未构建的 `ceiling` 档所用的、具备测试感知能力的 Simulated SME。这正是 `curated` 分支能在 skill 敏感的问题上*超越*可恢复上限（recoverable ceiling）的机制所在。
