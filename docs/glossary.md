...
| **Turn record** | What `stamp` projects, declared in `register/record.py`: `outcome`, `guardrail_errors`, `terminal_reason`, `execution` (the per-attempt ledger), `latency_sec`, and the treatment hashes. **There is no reliability stamp.** This row used to describe two axes, `safety_clearance` and `semantic_assurance`, that exist in no source file (audit §4.5). |
| **Graded delivery** | Re-execute a statement after a `COST`-layer refusal instead of hard-refusing (ADR 0006 §5). Ships disabled with the cost layer. Not “graded against gold”, and it no longer emits a `semantic_assurance` value because there is no such field. |
