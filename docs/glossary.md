...
| **Reliability stamp** | The two-axis reliability marking on a delivered answer (D5): `safety_clearance` (bool hard gate — guardrails + authorization passed) and `semantic_assurance` (`certified` / `heuristic` / `unverified` — how well-grounded). The legacy single-axis tier (`governed` / `lineage` / `fenced_raw` / `refused`) is kept only as a 1:1 compact projection. `certified` means safe + in-scope, **not** verified-correct, and is slated for rename + EX-calibration (Audit dispositions R2). |
...
| **Simulated SME** | An eval-harness **Responder**: an LLM briefed with a dataset's *domain meaning* (what tables/columns represent), phrased as SME knowledge rather than a de-obfuscation map, and answering **Clarification questions** one at a time. Never handed a held-out **test** question's gold SQL (the one hard leakage invariant). The *fair* Simulated SME is **train-bounded** (training questions + evidence only); a separate **test-aware** variant that also holds the test questions + evidence in its **retrieval index** (still never their gold SQL) is the oracle behind the **Recoverable ceiling**, reported only as an upper-bound line. It works via **BM25 + vector search tools** over the question bank (index scope = the fair↔ceiling knob *and* the leakage boundary), not a stuffed brief. Knowledge is **pull-based**: it answers only what the curator asks (may say it doesn't know, or add closely-related context), never volunteering the whole corpus — so the curator's questioning is part of what A3 measures. It voices reliability as natural domain advice ("unreliable — prefer `net_total`") and never uses benchmark words like *decoy* / *trap*. |
...
| **Recoverable ceiling** (test-aware SME oracle) | The eval's dashed upper-bound line: a **Simulated SME** that holds the held-out **test questions + evidence hints (never the test gold SQL)** in its retrieval index. A **deliberately-leaky oracle**, quarantined from the fair arms; `1.0 − ceiling` is the agent's irreducible SQL-gen error, `ceiling − SME` the test-relevant knowledge a train-bounded SME cannot reach. Replaces the **retired** de-obfuscation "gold semantic layer" (rename-map read-back → real names + decoy exclusions + FK graph), which was never a true ceiling since curator skills can exceed it. BIRD growth-benchmark concept. |
