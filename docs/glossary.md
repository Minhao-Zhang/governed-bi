...
> labels: `seeded` sits between the first two and `curated_sme_blind` between the
> last two. Use the arm names, not the letters; the `gold` arm /
...
| **Blind SME arm** (`curated_sme_blind`) | `curated` + the Simulated-SME clarification round with the SME **blind to BIRD's human `database_description` CSVs**. Opt-in (a full SME round per database). `curated → curated_sme_blind` is the clarification protocol on its own. |
| **Curated+SME arm** (`curated_sme`) | The same round with the human column documentation in the SME's brief. `curated_sme_blind → curated_sme` is what that documentation is worth. Without the blind rung the two are bundled, and the compound step says so. The growth axis. |
