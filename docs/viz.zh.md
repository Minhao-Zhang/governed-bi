...
面向 [Agentic BI System](system-overview.zh.md) corpus 的审计**驾驶舱**。本仓库提供的是
**只读**驾驶舱：浏览并审计 AI 构建的层，并可以提出问题以查看治理后的答案及其可靠性标记。
...
> `presenter.py` 保存与 UI 无关的视图模型；`app.py` 是 Streamlit 渲染器，是唯一与
> UI 相关的模块（可选的 `viz` extra）。

## 范围：引擎 vs 产品

驾驶舱的*读取*侧贴近引擎：是架设在 corpus 之上的开发/审计/演示工具。驾驶舱的*编辑*侧
...
| 只读驾驶舱（health / tables / assets / skills / ask） | 本仓库（`viz/`） |
...
本仓库内置（只读）、基于 corpus 计算得到的视图：

- **Chat**（默认）。对受治理 server 流程的多轮对话；每个答案都展示双轴标记、SQL 与
  溯源轨迹，追问会通过工作记忆（D8）回灌。
...
- **问答（Ask）**。同一条 server 流程的单轮表单形式（一问一答），展示标记、SQL 与
  护栏/规划轨迹。

设计愿景，尚未在本仓库实现（更完整的驾驶舱，或下游产品）：
...
这是一个基于 corpus 计算得到的本地只读应用：与 UI 无关的视图模型（`presenter`）加上
一个薄的 Streamlit 渲染层（`app`），因此前端可以替换，自身不携带任何业务逻辑。没有
