...
- **Built:** working memory is implemented as an in-process, session-scoped store (`InMemoryWorkingMemory`), the store the `before_model` middleware reads to inject prior context. Episodic, correction and profile memory are **not built** — and, as of 2026-07-28, not stubbed either: the empty `EpisodicMemory` / `CorrectionMemory` protocols and the `profile_ttl_days` / route-budget config were deleted, because an interface nothing implements and a threshold nothing reads are not a seam, they are a claim. Reinstate them from a design when eval earns them.
...
- **Structured-intent SQL cache.** ~~The semantic cache keys on an embedding-similarity gate, and a global cosine threshold is a weak equivalence test (two questions can differ in period, denominator, entity, or metric).~~ **Resolved by deletion (2026-07-28).** The cache was never wired into a caller: every entrypoint and every eval arm called the serve rails without it, so `cache_lookup` returned a miss unconditionally in production. Rather than fix the equivalence test on a fast path nobody used, `analyst/cache.py` and the `cache` graph node were removed. If latency ever justifies a cache, key it on structured intent (`corpus_release_hash + identity scope + metric ID + normalized dims/filters + join-plan fingerprint + policy version`) from the start.
...
- **Qualification is uniformly schema-qualified (supersedes the earlier mode-conditional design).** Every path emits fully-qualified `schema.table` SQL. The SQLite connector `ATTACH`es the database file under a schema alias (`corpus_pin` / BIRD `db_id`) so qualified names execute natively (`gateway/connectors/sqlite.py`); read-only is preserved via `PRAGMA query_only`. Cross-schema joins remain Postgres/Redshift-relevant at scale and stay un-graded by BIRD (below). `DataSourceConfig.serving_schema()` returns the ATTACH alias, the pinned Postgres schema, or `None` to span all schemas — the only variable is how many schemas exist and what a bare reference defaults to.
- **The guardrail is schema-qualified and remains the sole table-scoping gate.** Retrieval and the L4 license scope span all schemas: a **schema router** shortlists the relevant schemas, then expands **along curated joins** so a bridge table sitting in a third schema is not dropped (a similarity-only shortlist would cause *spurious* refusals indistinguishable from the honest one above). The L4 allow-set is fully-qualified `schema.table` membership; a bare reference resolves only to a designated default schema and is **refused as ambiguous** when the licensed set holds that name in more than one schema — this is what forbids a self-authorized off-scope schema. L3 keys are three-part `schema.table.column`; L5 union-find keys on `schema.table`. The licensed *id* set was already schema-correct (IDs embed the schema), so this is a projection fix. Read-only, the forced row cap, and the statement timeout are untouched — they live in the connectors, not the guardrail — and `search_path` is **not** used (L2 forbids `Command`); full qualification is the mechanism.
- **Alternatives:** a true three-level `connection → schema → table` model with cross-connection federation (rejected — one engine cannot join across physical connections; federation is a warehouse concern); auto-discovering cross-schema joins from FK metadata or name heuristics (rejected — cross-schema FKs rarely exist, and guessing them is the dominant error mode in FK-less settings); keeping SQLite bare while Postgres qualifies (rejected 2026-07-17 — dual conventions; ATTACH makes one qualified convention work on the graded path too).
- **Consequence:** refines **D1**'s target (multi-schema-capable within one database, tenancy still out) and **D9**'s corpus contract (`db` → `schema`; the `<db>/` subtree becomes `<schema>/`). **Cross-schema serving is un-graded by BIRD** (**D14**), an accepted, documented limitation covered instead by guardrail unit tests, a two-schema Postgres integration fixture, and a CI check for `(schema, physical_name)` uniqueness and non-ambiguous allow-set keys. **Status: Implemented** (increments 1–9 landed; uniform qualification supersession 2026-07-17). Shipped path: gateway span-all + schema-qualified L3/L4/L5; qualified SQL-gen on all dialects; missing-edge refusal; API wire rename (`db` → `schema`); server-side graph scoping; on-disk YAML rename; join-aware schema router; removal of the `multi_schema` mode flag. Still deferred: server `/search` (client Fuse remains default per Q6). **`corpus_pin`** unifies the BIRD `db_id` and the default write subtree. The LLM coarse-to-fine pruning pass stays deferred behind the pluggable generator seam.
- **Historical note (2026-07-17 supersession):** earlier text kept SQLite bare to protect BIRD EX. That branch is retired — see the ATTACH design above. The retired single-schema driver (`run_experiment.py`, removed 2026-07-31) had pinned `multi_schema=False`.
...
- **Governance moves from convention to construction.** Every data touch passes
  ...
  → writes an **append-only audit ledger** entry; `wrap_model_call` forces
  `parallel_tool_calls=False` and records token usage — it does **not** scope tools
  by identity; identity tool-scoping is an unimplemented seam). The agent never
  calls the gateway directly and never sets its own stamp: `safety_clearance` /
  `semantic_assurance` are computed by deterministic `finalize` code from what
  actually happened. Enforcement and audit share one interception point, so you
  can never execute (or refuse) without a record.
...
> every scored eval row, and `manifest.json` (`run_datalake.py`).
