...
**证据**：`proxy_full_opus_high_corpus30872d3.jsonl`，1351 行，逐题带 `licensed` / `gold_sql` /
`generated_sql` / `gold_fingerprint`
**引擎**：`d121c34`（origin/v2）　**语料**：`../BIRD-corpus` @ `30872d3`
...
## 八、弃权质量：这个引擎真正与众不同的地方

```
committed（引擎交付的答案）  n=1189   准确率 0.658
abstained（拒绝/澄清/capped） n= 162   若强行提交 0.204
                                      其中 79.6% 本来就会错
```
