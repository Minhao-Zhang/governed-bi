...
还带 `attempts` / `computed_correct` / `context_evicted` / `routing_pinned`，以及
...
**真 gold 上的表覆盖率是 0.923**（1130/1224 题的 gold 表全部被授权）。检索基本到位——
这一点很重要，因为它决定了下面每一桶该怎么读：覆盖不全的只有 94 道题，而引擎在这 94 道
上几乎全灭（只答对 8 道，其余 86 道就是上表「表覆盖不全」那一桶）。
...
**口径纪律**：对外报**未剔除的 EX**（与公开 BIRD 同口径），当前臂为 **0.664**。剔除退化题
...
**"弃权若强行提交"这一列只覆盖能定价的那部分。** v3-fold 的 86 次弃权里只有 69 次
能定价——另外 17 题数据集没发 gold fingerprint，引擎本会答对还是答错**不可知，不是零**。
精确率 0.812 是这 69 题上的数字，引用时必须带着这个分母。

交付集准确率是可定价弃权集的 **3.76 倍**（0.709 / 0.188）。若弃权是随机的，弃权集
应该也在 0.664 附近。

这与 EX 正交：一个 EX 更高的系统，其错误答案里没有任何信号告诉你是哪些。而这个引擎的
弃权行为不是叙事，是机制——23 个拒绝里 19 个终止在 `r_table_not_licensed`（check Layer 6
在报告检索失败），6 个澄清全部是授权集为空。**治理层在充当"我不知道"的检测器。**
...
一轮内的第二次及以后 = 1,963 次 = 59.3% 的调用，占全部输入 token 的 58.6%
```

这两个数都是从 `usage` 直接算出来的，不依赖对前缀长度的任何假设。

代理是 OpenAI 兼容的，长前缀自动缓存且不报缓存计数，所以这 58.6% 是靠 `model_calls`
算出来的，不依赖 provider。
...
- [The Death of Schema Linking?](https://arxiv.org/html/2408.07702) — 激进裁剪丢失 22.6%
  必需元素；本次 0.923 的覆盖率说明当前配置不在那个陷阱里
