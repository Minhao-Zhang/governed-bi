...
**Written 2026-08-03, before the adapters exist.** `ports.py`'s `Embedder` adapter list names
the adapters of the `Embedder` port. It read three when this file was written
(`model/openai_embedder.py`, `model/bedrock_embedder.py`, `model/deterministic_embedder.py`)
and names all four today, `model/proxy_embedder.py` having joined them. The deterministic one
is an adapter and not a courtesy fake, because it is what makes ADR 0005's implementation
steps 6–9 model-free.

**Do not edit this file to make it pass.** If a test looks wrong, stop and say so.

Every specification below that takes an `adapter` is parametrised over :data:`ADAPTERS`,
deliberately. One adapter is a hypothetical seam; the port earns its existence only if two
independent implementations satisfy one contract, and v1's fake discarded the arguments it
was handed, so a contract only the real adapter met would have caught nothing.
...
to "nothing scores" with no error anywhere. `retrieve/semantic.py::cosine` now raises on a width
...
#: The two adapters this file drives, by import path — **not** all four: `bedrock` and `proxy`
#: each need a provider tree and a credential to construct, and their identity surface is
#: covered offline by `tests/model/test_provider_selection.py` and
#: `tests/model/test_proxy_provider.py` instead. The OpenAI one must **skip loudly** without a
#: key, naming the environment variable and never printing its value; a silent skip is a green
#: suite that tested one adapter and reported two.
...
def test_embed_returns_one_vector_per_input_in_input_order(adapter: str) -> None:
    """`ports.Embedder.embed`'s ordering rule — *"``embed`` returns one vector per input, in order, each of length
    ``dimensions``."* A shorter or reordered result is a bug in the adapter, never something
    the caller reconciles.

    `retrieve/index.py::build_index` zips the returned vectors against the texts it asked for with
...
def test_an_empty_or_whitespace_input_refuses(adapter: str) -> None:
    """`ports.Embedder`'s no-blank-input rule — *"Callers must not pass empty/whitespace-only strings (adapters
    disagree on them)."* The disagreement is the hazard and is why it cannot be papered
    over: OpenAI accepts a blank and returns a vector that can score above zero and pollute
    a ranking, while Bedrock rejects it, so the same corpus defect surfaces as a ranking
    that is subtly wrong on one provider and a dead turn on the other.
...
def test_two_models_of_the_same_width_do_not_share_a_cache_entry() -> None:
    ...
    When this was written `build_index` took `vector_cache: Mapping[str, Vector]` keyed on
    the **summary text alone**, while `register/knobs.py`'s `embedding_model` knob said `embedding_model` is
    *"part of every vector cache key"*. Those two statements contradicted each other, and
    `cosine` raising on a width mismatch (`semantic.py::cosine`) closes only the case where the
    widths **differ**. The signature is now `vector_cache: VectorCache | None` and the key is
    `cache_key(text, model=…, dimensions=…)`; this body is what holds it there.
    ...
    **This is a specification about a signature, not only a behaviour.** If satisfying it
    ...
    # `semantic.py::cosine` raises when widths differ and these agree, so nothing downstream
...
def test_the_embedding_knobs_reach_knobs_resolved() -> None:
    ...
    `Role.comparability` (`register/knobs.py`'s `embedding_model` and `embedding_dimensions`
    rows), and when this was written nothing supplied either, because no embedder existed.
...
def test_the_deterministic_adapter_is_stable_across_processes() -> None:
    """`ports.py`'s `Embedder` adapter list names it as an adapter of the `Embedder` port, and it is what makes ADR
    0005's implementation steps 6–9 model-free: a third adapter, not a courtesy fake.
...
def test_the_semantic_channel_scores_a_facet_that_declares_it() -> None:
    ...
    `register/facets.py`'s `FACET_CHANNELS` gives `Stage.facet_example` only `Channel.semantic`. With no
    ...
    **Do not write this expecting a routing improvement — in the architecture this sentence
    was written against.** `register/citations.py`'s schema-shortlist citation records schema shortlist recall BM25
    0.736@1 / 0.844@3 versus embedding 0.694@1 / 0.852@3, at 57 schemas over all 1351
    questions, and BM25 **wins** at @1 there; the reason is on record too, that BIRD's
    ...
    That figure is a **single-channel shortlist ranking the 57 schema summaries directly**,
    and its own citation says to compare it only to another shortlist. Through v2's
    five-facet path, measured 2026-08-05 on `gold-semantic-layer-20260804`, the same citation
    records lexical-only at 0.5468@1 / 0.7018@3 against semantic-only at 0.9064@1 /
    0.9825@3 — so on the served topology the semantic channel is not neutral to routing. What
    this body asserts is narrower than either: a facet that declares only `Channel.semantic`
    can retrieve at all.
    """
    from governed_bi.model import DeterministicEmbedder
    from governed_bi.register.assets import AssetType
