...
Split out of ``test_agent_tools_hitl.py``, which was seven lines from ADR 0005 §6's hard cap at
1,000. Reuses that file's fixtures (``_state``, ``_tools``, ``_call``, ``_config``) bare --
``tests/`` carries no ``__init__.py``, so pytest's rootless import puts ``tests/serve/`` on
``sys.path`` and a sibling module in the same directory is importable by its bare name. That is
the same thing ``test_routing_replay_node.py`` already does to ``test_pass_two_and_context.py``.

``test_delivery_hash_stable_for_same_tool_payload`` and
``test_tool_bounds_from_state_includes_pulled_in`` come along rather than staying behind: both are
short, generic checks with no dependency on the ``ask_user``/HITL machinery that stayed, and
moving them is what let the boundary land under the cap without splitting the ``sample_rows``
cluster itself.
"""
