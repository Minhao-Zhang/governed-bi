"""A two-arm run was spent to learn its MDE was 9.6pp -- afterwards.

The treatment moved a population of 9 questions out of 131. No effect that population could
produce was detectable, and the run reported "no change" as though that were a measurement of the
feature rather than of the sample size. This refuses that run up front. The run is
RyanChenJung/governed-bi-utkuai@12c3e15's, not this repository's; ``eval/power.py``'s docstring
says so and says why the figures are kept.

``eval/power.py`` holds the gate and not the formula: the MDE is
:func:`governed_bi.measure.stats.mde`, the declared singleton, and ``power.py`` used to restate it
under a synonym with hardcoded z-constants. These tests therefore anchor the *singleton* against
that run's published figure and assert that the gate built on it still refuses that run's arm --
the two claims a delegation could break independently.
"""
...
def _discordance(discordant: int, n: int) -> Measured[float]:
    """That run's inputs in the shape ``stats.mde`` takes: a rate, not a count."""
    return Measured.rate(discordant, n, what=f"discordance over {n} pairs")


def test_the_one_mde_reproduces_the_published_figure() -> None:
    """That run's SUMMARY reports ``McNemar: -0.0153 (p=0.8238, n=131, discordant=20,
...
def test_more_questions_lower_the_bar() -> None:
    """The ``beer_factory`` pair discorded on 20 of 131 (15.3%). Four times the n at the same
...
def test_an_arm_hypothesising_less_than_its_mde_is_refused() -> None:
    """That run's shape: a treatment reaching 9 of 131 questions cannot produce 9.6pp even if it
