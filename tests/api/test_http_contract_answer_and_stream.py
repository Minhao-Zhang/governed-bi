...
**Split out of ``test_http_contract.py``** once that file reached 990 lines against ADR 0005
§6's hard 1000-line cap (``tools/check_file_length.py``). That file's own module docstring states
...
``test_ask_user_outstanding_clarification_latch.py`` already uses on
``tests/serve/test_agent_tools_hitl.py``. ``_isolated`` is imported though never called by name:
    ...
    **`why` is asserted present, not non-empty, and that changed on 2026-08-15.** It used to be
    ...
    model = ScriptedChatModel(
        responses=[
            AIMessage(
                content="",
                # `basis` is required in this fork: the tool routes the answer by it (a
                # data-definition answer folds into the shared corpus, a ranking one stays
                # turn-scoped), so a call without it never reaches the interrupt.
